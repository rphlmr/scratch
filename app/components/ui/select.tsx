...
>(({ className, size = "default", children, asChild, ...props }, ref) => {
  return asChild ? (
    ...
    <SelectPrimitive.Trigger
      ...
      data-size={size}
      className={cn(
        "focus-visible:inset-ring-[3px] focus-visible:inset-ring-ring/50 flex *:data-[slot=select-value]:flex justify-between items-center *:data-[slot=select-value]:items-center gap-2 *:data-[slot=select-value]:gap-2 bg-transparent dark:bg-input/30 dark:hover:bg-input/50 disabled:opacity-50 shadow-xs px-3 py-2 border border-input aria-invalid:border-destructive focus-visible:border-ring rounded-md outline-none aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 w-fit data-[size=default]:h-9 data-[size=sm]:h-8 [&_svg:not([class*='size-'])]:size-4 [&_svg:not([class*='text-'])]:text-muted-foreground data-[placeholder]:text-muted-foreground text-sm *:data-[slot=select-value]:line-clamp-1 whitespace-nowrap transition-[color,box-shadow] disabled:cursor-not-allowed [&_svg]:pointer-events-none [&_svg]:shrink-0",
        className
      )}
      {...props}
    >
